...
async function getDb() {
  ...
  if (!dbPromise) {
    dbPromise = clientPromise.then(async (client) => {
      ...
      await Promise.all([
        users.createIndex({ discord_id: 1 }, { unique: true }),
        users.createIndex({ status: 1, expires_at: 1 }),
        users.createIndex({ authorized_guilds: 1 }),
        users.createIndex({ updated_at: -1 }),
      ]);
      return db;
    });
  }
...
export async function addAuthorizedGuild(discordId, guildId) {
  ...
  return db.collection('oauth_users').findOneAndUpdate(
    { discord_id: String(discordId) },
    {
      ...
      $set: {
        status: 'active',
        last_authorized_guild: String(guildId),
        updated_at: now,
        last_seen_at: now,
      },
    },
