...
export async function validateAuthorization(discordId) {
  ...
  try {
    const { row: current, accessToken } = await accessTokenFor(discordId, false);
    await getMe(accessToken);
    return current;
  } catch (error) {
    try {
      const { row: refreshed, accessToken } = await accessTokenFor(discordId, true);
      await getMe(accessToken);
      return refreshed;
    } catch (secondError) {
      await setStatus(discordId, 'reauth_required');
...
export async function joinGuild(discordId, guildId) {
  ...
  const res = await fetch(guildPath, {
    method: 'PUT',
    headers: {
  ...
  if (!res.ok) {
    const err = new Error(data?.message || `Join failed (${res.status})`);
...
export async function removeRoleIfPresent(guildId, userId, roleId) {
  try {
    ...
    // Deliberately ignore if the member is not in the guild or role is absent.
  }
}
